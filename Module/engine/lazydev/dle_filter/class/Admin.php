...
class Admin
{
    ...
    static function row($title = '', $description = '', $field = '', $helper = '')
    {
		...
		$helper = $helper ? '<i class="help-button visible-lg-inline-block text-primary-600 fa fa-question-circle position-right" data-rel="popover" data-trigger="hover" data-placement="right" data-content="' . $helper . '" data-original-title="" title=""></i>' : '';
        echo '<tr>
    ...
    static function input($data)
    {
        ...
        if ($data[1] == 'range') {
            ...
            $class = ' w-9';
            $inputElemet .= $data[5] >=0 ? ' min="' . $data[5] . '"' : '';
    ...
    {
        return '<select name="' . $name . '" class="form-control custom-select chosen" data-placeholder="' . $placeholder . '" multiple>' . $select . '</select>';
    }
